...
class CellAutomatonWidget(Widget):
    def __init__(self, ellipse_size, ellipse_offset, window_width=640, window_height=400, rule=90,):
        ...
        self.graphic_columns = int((Window.size[0]-self.menu_width)/self.ellipse_box_size)
    ...
    def set_initial_display(self):
        btn1 = kb.Button(text="Random Rule", pos=(0, Window.size[1] - 100), size=(self.menu_width, 100))
        btn1.bind(on_press=self._draw_random_graphic_controller)
        self.add_widget(btn1)

        self.size_label = Label(text="Size: " + self.graphic_columns.__str__(), pos=(0, Window.size[1] - 150), size=(self.menu_width, 50))
        self.size_label.color = [1, 0, 0, 1]
        self.add_widget(self.size_label)

        size_input = TextInput(pos=(0, Window.size[1]-200), size=(self.menu_width, 50))
        size_input.bind(text=self.size_text_input_controller)
        self.add_widget(size_input)

        self.iterations_label = Label(text="Iterations: " + self.graphic_rows.__str__(), pos=(0, Window.size[1] - 250), size=(self.menu_width, 50))
        self.iterations_label.color = [1, 0, 0, 1]
        self.add_widget(self.iterations_label)

        iterations_input = TextInput(pos=(0, Window.size[1]-300), size=(self.menu_width, 50))
        iterations_input.bind(text=self.iterations_text_input_controller)
        self.add_widget(iterations_input)

        self.rule_label = Label(text="Rule: " + self.cell_automaton.rule.__str__(), pos=(0, Window.size[1] - 350), size=(self.menu_width, 50))
        self.rule_label.color = [1, 0, 0, 1]
        self.add_widget(self.rule_label)

        rule_input = TextInput(pos=(0, Window.size[1]-400), size=(self.menu_width, 50))
        rule_input.bind(text=self.rule_text_input_controller)
        self.add_widget(rule_input)

        draw_btn = kb.Button(text="Draw", pos=(0, Window.size[1] - 500), size=(self.menu_width, 100))
    ...
    def _draw_random_graphic_controller(self, instance):
        self.clear_graphic()
        self.create_graphic()
        self.cell_automaton.set_initial_state()
        self.cell_automaton.set_rule((self.cell_automaton.rule+30) % 250)
    ...
    def _get_graphic_cell_x_pos(self, column):
        return self.menu_width+column * self.ellipse_box_size
