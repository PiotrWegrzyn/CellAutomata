...
class CellAutomaton2D(CellAutomaton1D):
    ...
    def change_rows_count(self, rows_count):
        self._set_rows_count(rows_count)
        self._fit_to_size()
    ...
    def print_stats(self):
        super().print_stats()
        print("Rows: ", self.rows_count.__str__())
    ...
    def create_empty_state(self):
        return generate_empty_2d_list_of_list(self.rows_count)

    def _set_cells(self):
        processes = int(self.rows_count / 3)
        if processes <= 0:
            processes = 1
